...
def doit(args):
    ...
    if test.lower().startswith("diac"):
        ...
        repDiac = list(filter(lambda x: x in builder.uids(), (0x0A81, 0x0A82, 0x0A83, nukta)))
        repBase = list(filter(lambda x: x in builder.uids(), (0x0A95, 0x0AAA, dotted_circle)))
    ...
    if test.lower().startswith("conjuncts"):
        ...
        for c1 in consonants:
            ...
            ftml.closeTest()
        ftml.startTestGroup('Consonants (nuktas) with rakaar')
        for c1 in consonants:
            for c2 in consonants:
                builder.render((c1,nukta,virama,ra,c2,nukta), ftml, label=f'{c1:04X}', comment=builder.char(c1).basename)
            ftml.closeTest()

        ftml.startTestGroup('Consonants with rakaar and a below matra')
        for c1 in consonants:
        ...
        for c1 in consonants:
            ...
            ftml.closeTest()
        ftml.startTestGroup('Two consonants (nuktas)')
        for c1 in consonants:
            for c2 in consonants:
                builder.render((c1,nukta,virama,c2,nukta), ftml, label=f'{c1:04X}', comment=builder.char(c1).basename)
            ftml.closeTest()

        ftml.startTestGroup('Three consonants')
        for c1 in consonants:
            for c2 in consonants:
                builder.render((c1,virama,c2,virama,c1), ftml, label=f'{c1:04X}', comment=builder.char(c1).basename)
            ftml.closeTest()
        ftml.startTestGroup('Three consonants (nuktas)')
        for c1 in consonants:
            for c2 in consonants:
